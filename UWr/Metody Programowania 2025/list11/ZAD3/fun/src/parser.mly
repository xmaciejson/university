...
(* Funkcje pomocnicze do lukru składniowego *)
...
mexpr:
    ...
    | FUN;
      ...
      ARR; e = mexpr
        { node $loc (Fun(x, tp, e)) }
    | FUNREC; f = IDENT; args = fun_args; COLON; ret = type_simpl; ARR; body = mexpr
        { expand_funrec $loc f args ret body }
    | e = expr
        { e }
    ;

fun_args:
    | LPAREN; x = IDENT; COLON; tp = type_expr; RPAREN; rest = fun_args
        { (x, tp) :: rest }
    | LPAREN; x = IDENT; COLON; tp = type_expr; RPAREN
        { [(x, tp)] }
    ;
