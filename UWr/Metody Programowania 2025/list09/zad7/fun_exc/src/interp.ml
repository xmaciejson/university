...
let rec eval_env (env : env) (e : expr) : value comp =
  ...
  let rpn_to_ast (tokens : string list) : expr =
    ...
    let rec loop stack tokens =
      ...
      | tok :: rest ->
          ...
          | Some n -> loop (Int n :: stack) rest
          | None ->
              (* operator: we pop 2 args *)
              match stack with
              | rhs :: lhs :: tail ->
                  let op = op_of_string tok in
                  let expr = Binop (op, lhs, rhs) in
                  loop (expr :: tail) rest
              | _ -> failwith "Too few operands for operator"
    in
    loop [] tokens
